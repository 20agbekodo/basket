...
// Real photo with graceful fallback to pixel art
export function PlayerImg({ player, size, className, style }: {
  player: Player; size: number; className?: string; style?: React.CSSProperties
}) {
  const want = player.photoUrl ?? makeAvatar(player, size)
  const [src, setSrc] = useState(want)
  useEffect(() => { setSrc(player.photoUrl ?? makeAvatar(player, size)) }, [player.id, player.photoUrl, player, size])
  return (
...
export function estimateSalary(f: AddForm, players: Player[]): SalaryResult {
  ...
  let mid = Math.max(0.02, Math.min(58, vi * mult * 0.22))
...
export function AddPlayerModal({ players, onCancel, onSubmit }: {
  ...
  const pickPhoto = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    r.onload = () => setF(s => ({ ...s, photo: r.result as string }))
  ...
  const randomize = () => {
    // ensure players is used (for comp purposes)
    void players.length
